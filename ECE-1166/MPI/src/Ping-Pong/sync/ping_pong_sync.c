 ...
 * of a parallel computing architecture utilizing synchronous functions.
 ...
 * command: $ sbatch ping_pong_sync.slurm
 */
...
int main(int argc, char **argv) {
  ...
  if (argc != 2) {
    printf("Usage:\n\t\tmpirun -np 2 ./ping_pong_sync.x <data_size>");
    exit(EXIT_FAILURE);
  }
  ...
  if (world_rank == PING) {
    ...
    printf("-------- SYNCHRONOUS MPI COMMUNICATION --------\n");
